...
/* ============================================================
   ...
   LogEntry = {
     ...
     protein 额外字段:  { grams, food, source:'photo'|'quick'|'manual'|'legacy' }
     water 额外字段:    { ml }
     training 额外字段:  {
...
var QWEN_RELAY = (/\.netlify\.app$/i.test(location.hostname) || /\.github\.io$/i.test(location.hostname))
...
function defaultState(){
  ...
  return { v:5, settings:{
    proteinTarget:DEFAULT_PROTEIN, waterTarget:DEFAULT_WATER, sound:true, voice:true, motion:true,
...
/* 存过数据却读不懂 → 只读模式：不写、不覆盖，等用户导出原文 */
storageReadOnly = !__boot.state && __boot.hadData;
try{ lastPersistedStateJson = JSON.stringify(S); }catch(e){ lastPersistedStateJson = null; }
...
function loadVoiceBuffer(key){
  if(voiceBufCache[key]){ return Promise.resolve(voiceBufCache[key]); }
...
function collectWorkoutVoiceKeys(){
  var keys = ['countdown-start','countdown-3','switch-side',
    'finish-done','finish-stretch','finish-early',
    'encourage-1','encourage-2','encourage-3','encourage-4','encourage-5'];
...
function speak(text, opts){
  ...
  if(opts.mp3Key && playVoiceMp3(opts.mp3Key, opts)){ return; }
  // 2) 无文本则不走 Web Speech（跟练中避免微信 TTS 失败弹窗）
...
function speakEncourage(){ speak(null, { mp3Key: 'encourage-' + (1 + Math.floor(Math.random() * ENCOURAGE.length)) }); }
function announceStep(step){
  if(!step){ return; }
  var key = null;
  if(step.type === 'warmup'){ key = resolveVoiceKey('warmup-', step.move); }
  else if(step.type === 'work'){ key = resolveVoiceKey('work-', step.move); }
  else if(step.type === 'stretch'){ key = resolveVoiceKey('stretch-', step.move); }
  else if(step.type === 'break'){ speakEncourage(); return; }
  else if(step.type === 'rest'){
    // 休息只保留提示音；微信里 Web Speech 不稳定，且无对应预生成音频
    return;
  }
  if(key){ speak(null, { mp3Key: key }); }
}
...
function tick(){
  ...
  if(step.type === 'work' && step.move.sideMode === 'half' && !sideChanged && realLeft <= Math.ceil(step.sec/2)){
    ...
    cueShort(); speak(null, { mp3Key: 'switch-side' });
  }
  ...
  if(left !== lastLeft){          // 每帧写 DOM 无意义，秒数变化时再更新
    ...
    if(left <= 3 && left >= 1){
      cueShort();
      if(left === 3 && !cdSpoken){ cdSpoken = true; speak(null, { mp3Key: 'countdown-3' }); }
    }
  }
...
function runCountdown(){
  ...
  unlockVoiceAudio();
  preloadVoiceKeys(collectWorkoutVoiceKeys());
  cueShort(); speak(null, { mp3Key: 'countdown-start' });
...
function finishWorkout(early){
  ...
  // 用预生成 mp3；没 mp3 的兜底分支（保存失败 / 不足 1 分钟）走 Web Speech
  var doneKey = didLog ? (early ? 'finish-early' : (isStretch ? 'finish-stretch' : 'finish-done')) : null;
  if(doneKey){ speak(null, { mp3Key: doneKey }); }
...
function renderToday(){
  ...
  $('tGo').hidden = done;
  $('tDone').hidden = !done;
  $('tAside').textContent = done ? '已完成' : '待完成';
}
...
function openPlanPreview(){
  ...
  if(!isStretch){
    body.insertAdjacentHTML('beforeend', '<div class="plan-sec"><span class="tag work">正式</span><div class="body"><div class="h"><span class="num">' + sm.workNames.length + '</span> 个动作 × ' + sm.rounds + ' 轮 · 约 ' + sec2m(sm.work) + '</div><div class="list">' + sm.workNames.map(escapeHtml).join(' → ') + '</div></div></div>');
  }
...
$('btnVoicePreview').addEventListener('click', function(){
  ...
  if(playVoiceMp3('encourage-1', { showError:true })){ return; }
  // 兜底：之前那套 Web Speech
  var name = $('setVoiceName').value;
  var zh = zhVoices();
  var v = null;
  if(name){ for(var i=0;i<zh.length;i++){ if(zh[i].name === name){ v = zh[i]; break; } } }
  if(!v){ v = pickVoice(); }
  if(!('speechSynthesis' in window) || !v){ showToast('没有可用的语音', 'error'); return; }
  try{
    window.speechSynthesis.cancel();
    var u = new SpeechSynthesisUtterance('你好，我是你的跟练伙伴。坚持住，就快完成了～');
    u.lang = 'zh-CN'; u.rate = 0.95; u.pitch = 1.10; u.voice = v;
    window.speechSynthesis.speak(u);
...
$('importFile').addEventListener('change', function(e){
  ...
  reader.onload = function(){
    try{
      ...
      var next = { v:5, settings:normalizeSettings(raw.settings), logs:normalizeLogs(raw.logs), weights:normalizeWeights(raw.weights) };
